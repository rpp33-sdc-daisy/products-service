...
const getAllProducts = "SELECT products.*, json_agg(json_build_object('id', features.id, 'feature', features.feature, 'value', features.value)) AS features from products JOIN features ON products.id=features.product_id GROUP BY products.id LIMIT 1;";
