...
const getAllProducts = "SELECT products.*, json_agg(json_build_object('id', features.id, 'feature', features.feature, 'value', features.value)) AS features from products JOIN features ON products.product_id=features.product_id GROUP BY products.product_id LIMIT 1;";
